    ...
    def plot_all(
                            ...
                            for quantity in quantities_to_ensure:
                                try:
                                    ...
                                    if quantity == specifiers.TEMPERATURE:
                                        ...
                                        a = 4.0 * dut_lib.dut_ref.length / dut_lib.dut_ref.width
                                        F_th = 1
                                        if a > 0.0:
                                            F_th = dut_lib.dut_ref.length / np.log(a)
        ...
        if create_doc:
            ...
            try:
                ...
                string_deckblatt = string_deckblatt.replace(
                    "_wafer", str(dut_lib.wafer).replace("_", r"\_")
                )
                string_deckblatt = string_deckblatt.replace(
                    "_date_TO", str(dut_lib.date_tapeout).replace("_", r"\_")
                )
                string_deckblatt = string_deckblatt.replace(
                    "_date_received", str(dut_lib.date_received).replace("_", r"\_")
                )
                string_deckblatt = string_deckblatt.replace(
