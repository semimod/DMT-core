...
def read_data( filename, key=None, **kwargs):
    ...
    if extension == '.mdm':
        ...
        raise IOError('Error: DMT can not open file ' + filename + ' since extension ' + extension + ' cannot be read.')
...
def read_mdm_blocks(np.ndarray[DTYPE_f, ndim=1] block, int n_row,int n_col,int n_iccap_vars, np.ndarray[DTYPE_f, ndim=1] iccap_vars):
    cdef np.ndarray[DTYPE_f, ndim=2] data_raw = np.zeros([n_row, n_col + n_iccap_vars ], dtype=np.float64)
    #read in raw data
...
def read_mdm(str filename):
    ...
    cdef:
        list block_data = []
        int  index_data = 0, n_row, n_col, n_iccap_vars, i
        str mdmtext
        np.ndarray block, data_raw
        dict iccap_Vars = {}
    ...
    for data in re.findall(r'^BEGIN_DB[\r\n]+([\s\S]*?)END_DB',mdmtext,re.MULTILINE):
        ...
        for i,line in enumerate(block_lines):
            ...
            if 'ICCAP_VAR' in line or 'USER_VAR' in line:
                try:
                    ...
                    raise ValueError("When trying to convert iccap_vars, ValueError. Could not convert to float.")
                #pass
            else:
                if i==0:
        ...
        data_raw      = read_mdm_blocks(block, n_row, n_col, n_iccap_vars, np.array([value  for key,value in iccap_vars.items()]))
        #read in raw data
...
def read_elpa(filename, header=2):
    ...
    columns       =  split_header[1:] #names of columns
        ...
        #try:
            ...
            #raise IOError('DMT->read_elpa: the number of columns and the length of the column names do not match. I tried to get more column names from the second line, but this failed.')
        #except ValueError:
    ...
    if n_row != round(n_row):
        warnings.warn('DMT -> Data_reader: Encountered a weird number of rows in {:s} (n_row = {:f}).'.format(filename, n_row))
            ...
            if nr_axes == 1:
                ...
                for i_y in range(0, len_axes[1]):
                    ...
                    for _i_x in range(0, len_axes[0]):
                        ...
                        read_vals.append(
                            obj_struct_f.unpack(my_file.read(4))[0])
                ...
                for i_z in range(0, len_axes[2]):
                    ...
                    for i_y in range(0, len_axes[1]):
                        ...
                        for _i_x in range(0, len_axes[0]):
                            read_vals[i_z][i_y].append(
                                obj_struct_f.unpack(my_file.read(4))[0])

                raise NotImplementedError('Needs testing how to flatten the list of lists of lists!')
